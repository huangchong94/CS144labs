...
int sr_find_next_hop_ip_and_iface(struct sr_rt *rt, uint32_t ip_dst, uint32_t *next_hop_ip_p, char *iface_out)
{
  struct sr_rt *default_entry = 0;
  struct sr_rt *match_entry = 0;
  uint32_t match_mask = 0;
  struct sr_rt *entry = 0;
  int found = 0;
  
  for (entry=rt; entry; entry=entry->next) {
    ...
    if (mask == 0) {
      ...
      if (ntohl(mask) > ntohl(match_mask) && (dest&mask) == (ip_dst&mask)) {
        match_entry = entry;
        match_mask = mask;
      }
    }
  ...
  if (match_entry || default_entry)
    found = 1; 
  if (found) {
    if (!match_entry)
      match_entry = default_entry;
    *next_hop_ip_p = match_entry->gw.s_addr;
