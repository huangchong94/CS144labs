...
#define DEFAULT_TCP_E_TIMEOUT 7440
#define DEFAULT_TCP_T_TIMEOUT 300
...
int main(int argc, char **argv)
{
    ...
    char *logfile = 0;
    uint8_t nat_on = 0;
    uint16_t icmp_timeout = DEFAULT_ICMP_TIMEOUT;
    uint16_t tcp_e_timeout = DEFAULT_TCP_E_TIMEOUT;
    uint16_t tcp_t_timeout = DEFAULT_TCP_T_TIMEOUT;
    struct sr_instance sr;

    printf("Using %s\n", VERSION_INFO);

    while ((c = getopt(argc, argv, "hns:v:p:u:t:r:l:E:I:R:T:")) != EOF)
    {
        switch (c)
        {
            case 'h':
    ...
    sr.nat.tcp_e_timeout = tcp_e_timeout;
    sr.nat.tcp_t_timeout = tcp_t_timeout;
